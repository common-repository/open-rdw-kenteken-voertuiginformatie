...
use App\OpenRdwFormatter as RdwFormatter;

function ordwAddExtensionMenu()
{
    $defaultMenus = [];
    $menus = apply_filters('ordwAddExtensionMenu', $defaultMenus);

    ob_start();
    include(ORK_PLUGIN.'/admin/partials/extensionhtml.php');
    $extensionHtml = ob_get_clean();
...
<div id="app-container">
    <section class="panel app open-rdw-kenteken-voertuiginformatie">
        <div id="header">
            <header class="panel-heading">
                ...
                <!-- | Status <?php echo get_option('status_puc_license_rdw') ?> | Next status check in <?php echo gmdate('H:i:s', get_option('puc_date_rdw') - time()) ?> | -->
                <div class="pull-right"><a href="#" class="btn btn-primary save-changes"><span class="dashicons dashicons-update"></span> <?php echo __('Save', 'tussendoor-rdw') ?></a></div>
                <div class="clearfix"></div>
                <ul class="nav nav-tabs">
                    <li class="active"><a href="#dashboard-doc"><span class="dashicons dashicons-dashboard"></span> Dashboard</a></li>
                    <li><a href="#formatters-doc"><span class="dashicons dashicons-admin-appearance"></span> Formatters</a></li>
                    <li><a href="#widget-doc"><span class="dashicons dashicons-admin-appearance"></span> Widget</a></li>
                    ...
                    <?php foreach ($extMenu as $id => $menu) : ?>
                        <li>
                            <?php echo $menu['before']; ?>
                            <a href="#<?php echo $id; ?>"><span class="<?php echo $menu['class']; ?>"></span> <?php echo $menu['name']; ?></a>
                            <?php echo $menu['after']; ?>
                        </li>
                    <?php endforeach; ?>
                </ul>
        ...
        <div class="panel-body">
            <div id="dashboard-doc">
                ...
                <div class="form">
                    <div class="row">
                        <div class="pull-left col-left"><label for="rdw_license"><?php echo __('License', 'tussendoor-rdw') ?></label></div>
                        <div><input type="text" id="rdw_license" placeholder="<?php echo __('License', 'tussendoor-rdw') ?>" value="<?php echo get_option('puc_license_rdw') ?>" /></div>
                    </div>
                    <h2>Dashboard:</h2>
                    <p><?php echo __('Thanks for using our Opendata RDW WordPress plugin. You can use this plugin in a couple of different ways and these are explained in more detail in the following tabs.', 'tussendoor-rdw') ?></p>
                    <p><?php echo __('You can display this plugin by using Open RDW widgets in your sidebar, next to widgets there is also a possibility to use the WYSIWYG editor to add a shortcode to your page or post (or different CPT). It is also possible to integrate the plugin with Contact Form 7.', 'tussendoor-rdw') ?></p>
                    <p><?php echo __('By using this plugin with Contact Form 7 you can easily create "stock forms". The functionality from Contact Form 7 stays the same, this means you can also add extra fields or attachments (pictures).', 'tussendoor-rdw') ?></p>
                </div>
                <hr>
                <div class="row" id="small-extension-list">
                    <h2 class="column column-100">Nieuw: uitbreidingen!</h2>
                    <div class="column column-25">
                        <a href="<?php echo admin_url('admin.php?page=open_data_rdw&tab=extensions'); ?>" title="Uitbreiding Post via kentekeninvoer">
                            Post via kentekeninvoer
                        </a>
                        <a href="<?php echo admin_url('admin.php?page=open_data_rdw&tab=extensions'); ?>" title="Uitbreiding Post via kentekeninvoer">
                            <img src="<?php echo ORK_PLUGIN_PATH; ?>/admin/images/addon_cpt_add.png">
                        </a>
                    </div>
                    <div class="column column-25 end">
                        <a href="<?php echo admin_url('admin.php?page=open_data_rdw&tab=extensions'); ?>" title="Uitbreiding reistijd berekenen">
                            Reistijd berekenen
                        </a>
                        <a href="<?php echo admin_url('admin.php?page=open_data_rdw&tab=extensions'); ?>" title="Uitbreiding reistijd berekenen">
                            <img src="<?php echo ORK_PLUGIN_PATH; ?>/admin/images/addon_tt_add.png">
                        </a>
                    </div>
                </div>
            ...
            <div id="formatters-doc" style="display: none;">
                <h2>Formatters:</h2>
                <?php $current = (new RdwFormatter)->all(); ?>
                <?php $fields = require ORK_PLUGIN.'/app/open-rdw-kenteken-voertuiginformatie-fields.php'; ?>
                <?php $availableTypes = [
                    'date_format:Y-m-d'     => 'Als datum ('.date('Y-m-d').')',
                    'date_format:Y/m/d'     => 'Als datum ('.date('Y/m/d').')',
                    'date_format:d-m-Y'     => 'Als datum ('.date('d-m-Y').')',
                    'date_format:d/m/Y'     => 'Als datum ('.date('d/m/Y').')',
                    // 'timestamp'             => 'Als timestamp ('.time().')',
                    'money'                 => 'Als geld (&euro; 1.234,56)',
                    'all_caps'              => 'IN HOOFDLETTERS',
                    'ucwords'               => 'Elk Woord Met Hoofdletter',
                    'ucfirst'               => 'Eerste woord met hoofdletter',
                    'str_lower'             => 'zonder hoofdletters',
                    // 'callback'              => 'PHP Functie',
                ]; ?>

                <h3>Nieuwe formatter toevoegen</h3>

                <?php
                    $categorizedFields = [];
                    array_walk($fields, function ($item, $key) use (&$categorizedFields) {
                        $categorizedFields[$item['category']][$key] = $item;
                    });
                ?>

                <form class="form" method="POST" id="formatters_form">
                    <div class="row">
                        <div class="pull-left col-left">
                            <label for="rdw_field_name">Veldnaam</label>
                        </div>
                        <div>
                            <select name="rdw_field_name">
                                <?php foreach ($categorizedFields as $categoryLabel => $fields) : ?>
                                    <optgroup label="<?php echo $categoryLabel; ?>">
                                        <?php foreach ($fields as $fieldName => $labels) : ?>
                    ...
                    <div class="row">
                        <div class="pull-left col-left">
                            <label for="rdw_formatter_type">Type</label>
                        </div>
                        <div>
                            <select name="rdw_formatter_type">
                                <optgroup label="Standaard formatters">
                                    <?php foreach ($availableTypes as $name => $label) : ?>
                                        <option value="<?php echo $name; ?>"><?php echo $label; ?></option>
                                    <?php endforeach; ?>
                                </optgroup>
                                <optgroup label="Voor ontwikkelaars">
                                    <option value="timestamp">
                                        Als timestamp (<?php echo time(); ?>)
                                    </option>
                        ...
                        <p>
                            <small><strong>Let op:</strong> Bij gebruik van een PHP functie worden er standaard twee argumenten meegegeven aan de gekozen functie: de waarde vanuit RDW en de naam van het veld. De gekozen functie moet dus minimaal twee parameters accepteren om te functioneren. <br> Een aantal gevoellige functies zijn standaard uitgesloten om misbruik te voorkomen. Denk bijvoorbeeld aan <code>exec()</code>, <code>eval()</code> en vergelijkbare functies.</small>
                        </p>
                    </div>

                    <div class="row">
                        <a href="#" class="btn btn-secondary add-formatter">
                            <span class="dashicons dashicons-plus"></span> Formatter toevoegen</a>
                    </div>


                    <div class="row">
                        <table class="formatter_table">
                            <tr class="formatter_table_header">
                                <th>Veldnaam</th>
                                <th>Type</th>
                                <th>Callback</th>
                                <th>Verwijderen</th>
                            </tr>
                            <tr style="display: none;" class="formatter_table_blueprint" data-row="">
                                <td class="formatter_field">

                                    <input type="hidden" name="formatter[-1][name]" value="">
                                </td>
                                <td class="formatter_type">

                                    <input type="hidden" name="formatter[-1][type]" value="">
                                </td>
                                <td class="formatter_callback">
                                    <input type="text" name="formatter[-1][callback]" placeholder="functie naam zonder haakjes" style="width: 100%;">
                                </td>
                                <td class="formatter_delete">
                                    <button class="btn btn-secondary delete-formatter">Verwijderen</button>
                                </td>
                            </tr>
                            <?php if (!empty($current)) : ?>
                                <?php $count = 0; ?>
                                <?php foreach ($current as $fieldName => $callback) : ?>
                                    <?php list($type, $callback) = $callback; ?>
                                    <tr data-row="<?php echo $count; ?>">
                                        <td class="formatter_field">
                                            <?php echo $fieldName; ?>
                                            <input type="hidden" name="formatter[<?php echo $count; ?>][name]" value="<?php echo $fieldName; ?>">
                                        </td>
                                        <td class="formatter_type">
                                            <?php echo isset($availableTypes[$type]) ? $availableTypes[$type] : $type; ?>
                                            <input type="hidden" name="formatter[<?php echo $count; ?>][type]" value="<?php echo $type; ?>">
                                        </td>
                                        <td class="formatter_callback">
                                            <?php if ($type == 'callback') : ?>
                                                <input type="text" name="formatter[<?php echo $count; ?>][callback]" placeholder="functie naam zonder haakjes" style="width: 100%;" value="<?php echo $callback; ?>">
                                            <?php else : ?>
                                                N.v.t.
                                            <?php endif; ?>
                                        </td>
                                        <td class="formatter_delete">
                                            <button class="btn btn-secondary delete-formatter">
                                                Verwijderen
                                            </button>
                                        </td>
                                    </tr>
                        ...
                        <style type="text/css">
                            ...
                            table.formatter_table td, table.formatter_table th {
                              border: 1px solid #dddddd;
                              text-align: left;
                              padding: 8px;
                            }

                            table.formatter_table tr:nth-child(odd) {
                        ...
                        <br><br>
                        <input type="submit" name="rdw_formatter_save" value="Formatters opslaan" class="btn btn-primary">
            ...
            <div id="widget-doc" style="display: none;">
                <img src="<?php echo plugin_dir_url(__FILE__) . '../images/widget_'.__('en', 'tussendoor-rdw').'.png' ?>" height="auto" width="auto" style="float: right; padding: 0 0 30px 30px;" />
                <h2><?php echo __('Add widget:', 'tussendoor-rdw') ?></h2>
                <p><?php echo __('To add the widget to your view please go to', 'tussendoor-rdw') ?> <a href="<?php echo admin_url('widgets.php'); ?>" target="_BLANK">widgets</a>. <?php echo __('Here you can find the Open RDW kenteken widget as an available widget. As soon as you add the widget to one of your sidebars it will show you more options.', 'tussendoor-rdw') ?></p>
                <p><strong><?php echo __('Widget title:', 'tussendoor-rdw') ?></strong> <?php echo __('Here you can fill in the widget title which will be displayed at the top of the widget.', 'tussendoor-rdw') ?></p>
                <p><strong>Widget class:</strong> <?php echo __('This class enables you to style the widget to your own needs.', 'tussendoor-rdw') ?></p>

                <ol>
                    <li><strong><?php echo __('Toggle all:', 'tussendoor-rdw') ?></strong> <?php echo __('As soon as you toggle the category checkbox, all child checkboxes will be checked as well. this enables you to easily select all the fields in one go.', 'tussendoor-rdw') ?></li>
                    <li><strong><?php echo __('Re-sort:', 'tussendoor-rdw') ?></strong> <?php echo __('As soon as you click on a category and keep your mouse pressed you can drag and drop your category. This allows you to configure the positions of the categories the way you want.', 'tussendoor-rdw') ?></li>
                    <li><strong><?php echo __('Open list:', 'tussendoor-rdw') ?></strong> <?php echo __('As soon as you click on the title of a category, it will slide down and show you all it\'s child fields. After doing this you can easily select the fields you would like to show on your website.', 'tussendoor-rdw') ?></li>
                    <li><strong><?php echo __('Re-sort:', 'tussendoor-rdw') ?></strong> <?php echo __('Just like you can sort the categories to your own needs it is also possible to sort the single fields within it\'s own category. This way you can display the fields on your website the way you want.', 'tussendoor-rdw') ?></li>
                </ol>
            </div>
            <div id="shortcode-doc" style="display: none;">
                <img src="<?php echo plugin_dir_url(__FILE__) . '../images/tinymce_'.__('en', 'tussendoor-rdw').'.png' ?>" height="auto" width="auto" style="float: right; padding: 0 0 30px 30px;" />
                <h3><?php echo __('Add shortcode:', 'tussendoor-rdw') ?></h3>
                <p><?php echo sprintf(__('As soon as you add or edit a page or post you will find the Open RDW button (%s) in your WYSIWYG menu. As soon as you hover your mouse over this button you will see the option "Insert shortcode".', 'tussendoor-rdw'), '<img src="'. plugin_dir_url(__FILE__) . '../images/open-rdw_grey.png'.'" height="13">') ?></p>
                <p><?php echo __('If you click on the button you will see a popup screen appear in which you can easily select which options you would like ot display on your page. As soon as you have checked the fields you would like to display you can click on the blue button "Add shortcode" to place the shortcode in your text area.', 'tussendoor-rdw') ?></p>
                <p><?php echo __('After saving your page or post your shortcode will be active immediately.', 'tussendoor-rdw') ?></p>
            ...
            <div id="contactform7-doc" style="display: none;">
                <img src="<?php echo plugin_dir_url(__FILE__) . '../images/cf7_'.__('en', 'tussendoor-rdw').'.png' ?>" height="auto" width="auto" style="float: right; padding: 0 0 30px 30px; width: 400px;" />
                <h3><?php echo __('Contact Form 7 integration:', 'tussendoor-rdw') ?></h3>
                <p><?php echo __('You can fully integrate our Open Data RDW plugin into your Contact Form 7 forms. As soon as you add or edit a form you will see a button called "Kenteken Open RDW". When you klick on this button you will see a popup screen which allows you to add the license plate field.', 'tussendoor-rdw') ?></p>
                <p><?php echo __('Next to adding the license plate field you also have some other options, you can also choose to add the brand, model and for example the MOT expiry date. You can easily copy and paste these fields into your form.', 'tussendoor-rdw') ?></p>
                <p><?php echo __('See below for an example form.', 'tussendoor-rdw') ?></p>
                <textarea rows="11" cols="40" readonly style="resize: none;" onclick="this.select();"><p><?php echo __('Search license', 'tussendoor-rdw') ?><br />&#13;&#10;[open_rdw open_rdw-ID]</p>&#13;&#10;&#13;&#10;<p><?php echo __('Brand', 'tussendoor-rdw') ?><br />&#13;&#10;[text merk]</p>&#13;&#10;&#13;&#10;<p>Model<br />&#13;&#10;[text handelsbenaming]</p>&#13;&#10;&#13;&#10;<p><?php echo __('MOT expiry', 'tussendoor-rdw') ?><br />&#13;&#10;[text vervaldatum_apk]</p></textarea>
                <p><strong><?php echo __('*Attention*:', 'tussendoor-rdw') ?></strong> <?php echo __('It is important to add the fields in both your form and mail template from Contact Form 7. If you don\'t do this you will not receive the filled in form data in your mail.', 'tussendoor-rdw') ?></p>
                <ol>
                    <li><strong><?php echo __('Open list:', 'tussendoor-rdw') ?></strong> <?php echo __('As soon as you click on the title of a category, it will slide down and show you all it\'s child tags.', 'tussendoor-rdw') ?></li>
                    <li><strong><?php echo __('Select tag:', 'tussendoor-rdw') ?></strong> <?php echo __('When you click on one of the tag boxes it will automatically select the tag and enable you to easily copy and paste the tag in your form.', 'tussendoor-rdw') ?></li>
                </ol>
            </div>
    ...
    <!-- <section class="panel feed">
        <div id="header">
            <header class="panel-heading">
                <h2 class="panel-title"><?php echo __('Recent news on Tussendoor.nl', 'tussendoor-rdw'); ?></h2>
            </header>
    ...
    <section class="panel feed">
        <div id="header">
            <header class="panel-heading">
                <h2 class="panel-title"><?php echo __('Like us on Facebook', 'tussendoor-rdw') ?></h2>
                <p><?php echo __('Like us on Facebook and stay tuned on all our updates.', 'tussendoor-rdw') ?></p>
            </header>
